defmodule Dynamic.Repo.Migrations.CreateConfigsTable do
  ...
  def change do
    create table(:configs, options: "INHERITS (base)") do
    ...
    execute("""
      ...
      VALUES (gen_random_uuid()::uuid, timezone('utc', now()), timezone('utc', now()), 'configs', 'base', (SELECT json_agg(json_build_object('name', t.column_name, 'type', t.udt_name)) FROM information_schema.columns AS t WHERE table_name = 'configs' AND t.table_schema = 'public'), '#{@system_id}', '#{@system_id}', NULL, '#{Jason.encode!(permissions)}', false, '#{Jason.encode!(acl_default)}');
    """)
  end
